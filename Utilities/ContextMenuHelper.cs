...
public static class ContextMenuHelper
{
        ...
        SpellCheckerService spellChecker)
    {
        if (isReadOnly)
            ...
            SetupContextMenu(textEditor, spellChecker);
    }

    private static void SetupContextMenu(ITextEditor textEditor, SpellCheckerService? spellChecker)
    {
        ...
        contextMenu.Opening += (_, _) =>
        {
            ...
            {
                var spellMenu = new MenuItem { Header = "Правописание" };

                if (textEditor.Visual is not TextEditor editorControl)
                {
                    spellMenu.Items.Add(new MenuItem
                    {
                        Header = "(редактор недоступен)",
                        IsEnabled = false
                    });

                    contextMenu.Items.Add(spellMenu);
                    return;
                }

                var offset = editorControl.CaretOffset;

                var word = GetWordAtOffset(textEditor.Text ?? string.Empty, offset);

                if (string.IsNullOrWhiteSpace(word))
                {
                    spellMenu.Items.Add(new MenuItem
                    {
                        Header = "(нет слова под курсором)",
                        IsEnabled = false
                    });
                }
                else
                {
                    var suggestions = spellChecker
                        .Suggest(word)
                    ...
                    if (suggestions.Count > 0)
                        foreach (var suggestion in suggestions)
                        {
                            ...
                            item.Click += (_, __) => { ReplaceWordAtOffset(textEditor, offset, suggestion); };

                            spellMenu.Items.Add(item);
                        }
                    else
                        spellMenu.Items.Add(new MenuItem
                        {
                            Header = "(нет вариантов)",
                            IsEnabled = false
                        });

                    spellMenu.Items.Add(new Separator());

                    var addToDict = new MenuItem
                    {
                        Header = $"Добавить \"{word}\" в словарь"
                    };
                    addToDict.Click += (_, __) => { spellChecker.AddWordToCSpell(word); };

                    spellMenu.Items.Add(addToDict);
                }
            ...
            excelsiorSansFontItem.Click += (s, e) =>
            {
                WrapInTag("<font=\"ExcelsiorSans SDF\">", "</font>", textEditor);
            };
    ...
    private static string GetWordAtOffset(string text, int index)
    {
        ...
        return left == right ? string.Empty : text[left..right];
    }

    private static void ReplaceWordAtOffset(ITextEditor editor, int offset, string newWord)
    {
        ...
        try
        {
            doc.Replace(left, right - left, newWord);
            editor.CaretOffset = left + newWord.Length;
        }
        finally
        {
            doc.EndUpdate();
